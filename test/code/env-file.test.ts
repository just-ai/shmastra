...
import {
    ...
    isValidEnvKey,
    formatEnvValue,
} from '../../src/shmastra/code/env-file'
...
describe('updateEnvContent', () => {
    ...
    it('quotes multiline values and decodes them on re-read', () => {
        const pem = '-----BEGIN KEY-----\nline2\n-----END KEY-----'
        const first = updateEnvContent('', { PEM: pem })
        expect(first.split('\n')).toHaveLength(2)
        expect(first.startsWith('PEM="')).toBe(true)
        // round-trip: second call should not change the encoded form
    ...
    it('strips inline comments from unquoted values on read', () => {
        // value "abc # note" must be read as "abc" and written back without the comment
